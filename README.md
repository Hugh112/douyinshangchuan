...
- 多浏览器账号独立配置、发布配额、独立图片/Excel/发布时间、掉号跳过和顺序切换；同一浏览器可重复加入队列。
...
- 随机图片或按文件名排序取第一张图片。
- 仅允许读取 `.xlsx`、`.xls`、`.xlsm` 文案文件；有表头和无表头均会正确使用第一条文案。
...
- `release/DouyinPublisher_Setup_v2.5.2.exe`
- `packages/DouyinPublisher_Update_v2.5.2.zip`
